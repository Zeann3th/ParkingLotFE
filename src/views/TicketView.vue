...
<template>
  <MenuLayout>
      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div v-else-if="itemList.length > 0"
          ...
          <div v-for="ticket in itemList" :key="ticket.id" class="
              ticket-card  relative <!-- Add relative for potential absolute positioning later -->
              !bg-white dark:!bg-gray-800
              rounded-lg shadow-sm hover:shadow-lg
              border border-gray-200 dark:border-gray-700
              flex flex-col overflow-hidden <!-- Ensure rounded corners clip children -->
            ...
            <div class="
                  ...
                  bg-gray-50 dark:bg-gray-700/50 <!-- Slightly different bg for stub -->
                  px-5 py-3   <!-- Padding for the stub content -->
                ">
        ...
        <Dialog v-model:visible="dialogs.view" modal :closable="true" :showHeader="false"
          ...
          <div v-if="selectedItem" class="p-5 md:p-6">
            <div class="grid grid-cols-3 gap-x-4 gap-y-4">
              ...
              <!-- Title -->
              <span class="col-span-1 text-sm font-medium text-gray-500 dark:text-gray-400">Title</span>
              <span class="col-span-2 text-sm text-gray-700 dark:text-gray-300 break-words">{{ selectedItem.title
              }}</span>

              <!-- Type -->
              <span class="col-span-1 text-sm font-medium text-gray-500 dark:text-gray-400">Type</span>
              <span class="col-span-2 text-sm text-gray-700 dark:text-gray-300">{{ getTypeLabel(selectedItem.type)
              }}</span>

              <!-- Status -->
              <span class="col-span-1 text-sm font-medium text-gray-500 dark:text-gray-400">Status</span>
              <span :class="getStatusTextClasses(selectedItem.status)" class="col-span-2 text-sm">
                {{ selectedItem.status }}
              </span>
          ...
          <div
            ...
            <Button label="Edit" icon="pi pi-pencil" class="p-button-sm p-button-outlined
              ...
              @click="() => console.log('Edit clicked', selectedItem?.id)" />
    ...
    <FloatingButton icon="+" @click="() => console.log('Clicked')" aria-label="Add new ticket" />
  </MenuLayout>
</template>
